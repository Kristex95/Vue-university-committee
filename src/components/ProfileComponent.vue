  ...
  <div class="d-flex justify-content-center flex-column align-items-center container w-75">
    ...
    <form class="border rounded w-75 p-2 bg-white" @submit.prevent="updateUser">
      ...
      <div class="row ms-1">
        ...
        <div v-else class="col-6">
          <input type="number" class="form-control" min="1" max="200" v-model="editedUser.school_mark" required>
        </div>
      </div>
      <div class="row ms-1">
      ...
      <div class="row mx-1 mt-2" v-if="!updateUserBool">
        <div v-if="!user.confirmed" class="bg-warning rounded">Not confirmed</div>
        <div v-else-if="user.confirmed" class="bg-success rounded text-white">Confirmed</div>
      </div>
      <div class="ms-3 mt-2">
